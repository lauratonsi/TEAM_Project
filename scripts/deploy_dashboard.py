...
<div id="va-widget" class="va-widget" role="dialog" aria-label="Virtual Analyst" aria-modal="false" hidden>
    <div class="analyst-panel va-panel">
        ...
        <details class="analyst-details">
            <summary>How does it work? — RAG + metadata filtering</summary>
            <p>
                The Virtual Analyst combines two layers. The <b>reference RAG</b> (run locally) indexes the
                validated XML into text chunks and answers with hybrid <b>BM25 + vector search</b>
                (FAISS + <code>all-MiniLM-L6-v2</code>) fused via <b>Reciprocal Rank Fusion</b>.
                On the published site it runs a <b>metadata-filtered</b> engine entirely in your browser:
                queries are parsed for structured dimensions — macro-region (<code>@region</code>, UN&nbsp;M49),
                currency (<code>@currency</code>) and venue type (<code>@category</code>) — to filter cities and
                venues <i>exactly</i>, then returns guaranteed values (coordinates, appeal score, indicators)
                straight from the injected Schema.org metadata, with no LLM and no hallucinated numbers.
            </p>
    ...
    <div class="pipeline-step">
      ...
      <div class="step-body">
        ...
        <p>
          ...
          costruisce <b>336 chunk testuali</b> tematici (trasporti, hotel, distretti, attrazioni,
        ...
        <h4 style="margin:14px 0 4px;color:var(--accent-dark)">Metadata-filtered retrieval — la fusione dei moduli</h4>
        <p>
          Il punto in cui RAG e <b>Web of Data</b> si fondono: <code>ingest.py</code> non indicizza solo
          testo, ma allega a ogni chunk i <b>metadati strutturati</b> iniettati nell'XML (gli stessi esposti
          come microdata Schema.org). <code>parse_filters()</code> analizza la domanda per dimensioni esatte
          e <code>hybrid_search()</code> <b>pre-filtra</b> i candidati prima della ricerca semantica:
        </p>
        <ul style="margin:6px 0;font-size:0.9rem">
          <li><b>Filtraggio esatto</b> su <code>@region</code> (UN&nbsp;M49), <code>@currency</code> e
              <code>@category</code> dei locali — niente più «zuppa di tag» da indovinare nel testo
              <em>(es. «nightclubs in non-euro cities» → solo nightlife di città non-euro)</em>.</li>
          <li><b>Disambiguazione</b>: un pub o una discoteca derivano dalla categoria tipizzata
              (<code>BarOrPub</code> / <code>NightClub</code>), non da pattern sulle frasi.</li>
          <li><b>Valori numerici garantiti</b>: coordinate (<code>GeoCoordinates</code>), appeal
              (<code>AggregateRating</code>) e indicatori sono restituiti verbatim dai metadati — zero
              allucinazioni sui numeri.</li>
        </ul>
        <p style="font-size:0.82rem;color:var(--slate-500);margin:8px 0 0;">
          La <b>stessa logica gira su GitHub Pages</b>: poiché l'hosting statico non può eseguire il backend
          FastAPI, il sito esegue il motore <code>metadataQuery()</code> direttamente nel browser sui dati
          già serializzati, ottenendo gli stessi filtri esatti senza alcun server.
        </p>
        <p style="font-size:0.82rem;color:var(--slate-500);margin:8px 0 0;">
